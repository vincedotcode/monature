...
import { setSelectedImage, setUploadedImagePath } from "@/redux/features/imageUploadSlice";
...
const ImageUploadModal: React.FC = () => {
  ...
  const selectedImage = useAppSelector((state) => state.imageUpload.selectedFile);
  const uploadedImagePath = useAppSelector((state) => state.imageUpload.uploadedImagePath);
    ...
    <Dialog>
      ...
      <DialogContent>
            ...
            >
              {loading && (
                <div className=" text-center max-w-md  ">
                  ...
                  <p className=" text-xs text-gray-400">
                    Do not refresh or perform any other action while the picture is being upload
                  </p>
                </div>
              )}
              ...
              {uploadedImagePath && !loading && (
                <div className="text-center">
                  ...
                  <p className=" text-xs text-gray-400">Click submit to upload the picture</p>
                </div>
              )}
            </label>
        ...
        <DialogFooter className=" flex items-center justify-end gap-x-2">
          ...
          <DialogClose asChild>
            <Button disabled={!selectedImage || loading} size={"sm"} className=" text-sm">
